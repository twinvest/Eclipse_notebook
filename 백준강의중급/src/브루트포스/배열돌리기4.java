...
public class 배열돌리기4 {
    ...
    static void go(int[][] a, Tuple t) {
        ...
        for (int s=1; s<=size; s++) {
            ...
            Collections.rotate(group, 1); //rotate연산은 [1,2,3,4,5] ==> [5,1,2,3,4]
            int index = 0;
            // (r-s, c-s) -> (r-s, c+s)
            for (int r=row-s, c=col-s; c<col+s; c++) {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            //위에 for문을 통해 모든 회전 연산을 수행 후
            //각각의 행들을 계산하여 제일 최소를 정답으로 갱신한다.
            for (int i=0; i<n; i++) {
                int sum = 0;
                for (int j=0; j<m; j++) {
                    sum += b[i][j];
        ...
        } while (next_permutation(d)); //next_permutation함수를 통해 d의 다음순열을 만든다.
