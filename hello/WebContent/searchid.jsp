...
%>
	...
	String dbURL = "jdbc:mysql://localhost:3306/shopdb";
		...
		sql = "select id from Member where name = ? and jnum = ?"; 
	...
	
{
		...
		%>
			...
			<%= name%>
			님의 ID는 "<%=result.getString("id") %>" 입니다. <br>
			<br> <input type=button value="로그인 하러가기" target="main"
				onClick="location.href='loginForm.html'"> <input type=button
				value="비밀번호 찾기" target="main"
				onClick="location.href='searchpw.html'"> <input type=button
				value="이전으로" target="main" onClick="location.href='searchid.html'">
	...
	%>
			...
			<% out.println("가입된 ID가 없습니다"); %>
			<input type=button value="이전으로" target="main"
				onClick="location.href='searchid.html'">
