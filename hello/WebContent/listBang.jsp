...
-->
		...
		<div id="jb-content">
			...
			<%
				...
				ArrayList<BangEntity> list = brddb.getBangList((currentPage - 1) * pageRecordNo, pageRecordNo);
				int row = 0;
			%>
			<br /></br/> <br />
			<table>
				...
				<tr bgcolor=<%=color%>
					...
					<td align=left>
						...
						%> <a
						href="readBang.jsp?num=<%=brd.getNum()%>&page=<%=currentPage%>"><%=brd.getTitle()%></a>
					</td>
			...
			<a href="listBang.jsp?page=<%=prePage%>">[이전]</a>
			...
			<%
				...
				for (int i = startPage; i <= endPage; i++) {
					if (i != currentPage) {
			%>
			<a href="listBang.jsp?page=<%=i%>"><%=i%></a>
			<%
				} else {
			%>
			<%=i%>
			<%
				...
				if (endPage < totalPage) {
					nextPage = endPage + 1;
			%>
			<a href="listBang.jsp?page=<%=nextPage%>">[이후]</a>
			<%
				}
			%>
			...
			<p>
				조회된 게시판 목록 수가
				<%=totalRecordNo%>개 입니다. <br> <br> <br> <Br>
			<form name=form method=post action=editBang.jsp>
