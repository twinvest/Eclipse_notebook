...
function deleteConfirm(num,pageNo) {
	...
	ok = confirm("삭제하시겠습니까?");
	if (ok) {
		location.href = "passwordBang.jsp?num=" + num  + "&page=" + pageNo ;
	} else {
		return;
	}			
}
						...
						<table>
							...
							<tr>
								<td colspan=4 align=center><input type=button value="수정"
									onClick="location.href='editBang.jsp?num=<%=num%>&page=<%=pageNo%>&menu=update'">
									<input type=button value="삭제"
									onClick="deleteConfirm(<%=num%>, <%=pageNo%>)"> <!-- 목록보기 버튼은 listbang.jsp로 이동 -->
