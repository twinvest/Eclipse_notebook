...
public class BoardDAO
{
    ...
    public ArrayList<BoardEntity> getBoardList() {
        this.connect();
        final ArrayList<BoardEntity> list = new ArrayList<BoardEntity>();
        final String SQL = "select * from Board order by ref desc, step";
        try {
            this.pstmt = this.con.prepareStatement(SQL);
            this.rs = this.pstmt.executeQuery();
            while (this.rs.next()) {
                final BoardEntity brd = new BoardEntity();
    ...
    public BoardEntity getBoard(final int num) {
        this.connect();
    ...
    public boolean insertDB(final BoardEntity board) {
    ...
    public boolean updateDB(final BoardEntity board) {
        boolean success = false;
        this.connect();
        final String sql = "update Board set name=?, title=?,  content=? where num=?";
        try {
            (this.pstmt = this.con.prepareStatement(sql)).setString(1, board.getName());
            this.pstmt.setString(2, board.getTitle());
            this.pstmt.setString(3, board.getContent());
            this.pstmt.setInt(4, board.getNum());
    ...
    public boolean insertReply(final BoardEntity board) {
        boolean success = false;
        int num = 0;
        int step = 0;
        this.connect();
        try {
            String sql = "select min(step) as tstep from Board where ref = ? and step > ? and depth <= ?";
            ...
            for (int r = depth - 1; r >= 0; --r) {
                sql = "select max(step) as tstep from Board where ref = ? and depth = ? and step < ?";
                (this.pstmt = this.con.prepareStatement(sql)).setInt(1, board.getRef());
                ...
                sql = "update Board set childCount = childCount + 1  where ref = ? and depth = ? and step = ? ";
                (this.pstmt = this.con.prepareStatement(sql)).setInt(1, board.getRef());
                this.pstmt.setInt(2, r);
    ...
    public boolean isPasswd(final int num, final String passwd) {
        boolean success = false;
        this.connect();
        final String sql = "select passwd from Board where num=?";
        try {
            (this.pstmt = this.con.prepareStatement(sql)).setInt(1, num);
            (this.rs = this.pstmt.executeQuery()).next();
    ...
    public boolean updateHit(final int num, final int hit) {
        boolean success = false;
        this.connect();
        final String sql = "update Board set hit=?  where num=?";
        try {
    ...
    public int deleteDB1(final int num) {
        int retval = 0;
        this.connect();
        try {
            String sql = "select * from Board where num = ?";
    ...
    public ArrayList<BoardEntity> getBoardList(final int startRecord, final int pageRecordNo) {
        this.connect();
        final ArrayList<BoardEntity> list = new ArrayList<BoardEntity>();
        final String sql = "select * from Board order by ref desc, step limit " + startRecord + ", " + pageRecordNo;
    ...
    public int getTotalRecNo() {
        int totalRecNum = 0;
        this.connect();
        final String sql = "select count(*) as cnt from Board";
        try {
            this.pstmt = this.con.prepareStatement(sql);
            this.rs = this.pstmt.executeQuery();
            if (this.rs.next()) {
